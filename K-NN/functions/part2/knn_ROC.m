...
for k=1:length(params.k_range)
    params.k = params.k_range(k);
    y_est =  knn(X_train,  y_train, X_test, params);
    FN = sum((y_test~=y_est) & y_test==1)/length(y_test);
    FP = sum((y_test~=y_est) & y_test==2)/length(y_test);
    TN = sum((y_test==y_est) & y_test==2)/length(y_test);
    TP = sum((y_test==y_est) & y_test==1)/length(y_test);
    TP_rate(1,k) = TP/(TP+FN);
    FP_rate(1,k) = FP/(FP+TN);
end
